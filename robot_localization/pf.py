...
class ParticleFilter(Node):
    ...
    def __init__(self):
        ...
        self.norm_mean = 0
        self.norm_std = 0.33
        self.odom_lin_noise = 0.1
        self.odom_ang_noise = 0.02
        self.laser_range_noise = 0.1
        self.resample_lin_noise = 0.5
        self.resample_ang_noise = 0.1
        self.pose_proportion = 0.035
        self.resample_proportion = 0.4
    ...
    def update_robot_pose(self):
        ...
        # Simple mean weight of top thresh weighted particles
        self.particle_cloud.sort(key=lambda x:x.w, reverse=True)
        x=0
        y=0
        theta = 0
        thresh = round(self.pose_proportion*self.n_particles)
        counter = 0
    ...
    def update_particles_with_odom(self):
        ...
        # Apply delta to all particles
        samples = sp.norm.rvs(loc = self.norm_mean, scale = self.norm_std, size = self.n_particles*3)
        for index, particle in enumerate(self.particle_cloud):
            particle.x += delta[0]+self.odom_lin_noise*samples[3*index]
            particle.y += delta[1]+self.odom_lin_noise*samples[3*index+1]
            particle.theta += delta[2]+self.odom_ang_noise*samples[3*index+2]
    ...
    def resample_particles(self):
        ...
        proportion = round(self.resample_proportion*self.n_particles)
        ...
        for p,t in zip(self.particle_cloud, thetas):
            samples = sp.norm.rvs(loc = self.norm_mean, scale = self.norm_std, size = 3)
            p.x += self.resample_lin_noise*samples[0]
            p.y += self.resample_lin_noise*samples[1]
            p.theta += self.resample_ang_noise*samples[2]
    ...
    def update_particles_with_laser(self, r, theta):
        ...
        samples = sp.norm.rvs(loc = self.norm_mean, scale = self.norm_std, size = len(r))
        for p in self.particle_cloud:
            ...
            for i, (ri, ti) in enumerate(zip(r,theta)):
                if(math.isfinite(ri)):
                    ang = p.theta+ti
                    ri_adj = ri+self.laser_range_noise*samples[i]
                    closest = self.occupancy_field.get_closest_obstacle_distance(p.x+ri_adj*math.cos(ang), p.y+ri_adj*math.sin(ang))
    ...
    def initialize_particle_cloud(self, timestamp, xy_theta=None):
        ...
        # Add particles in the bounding box of the map
    ...
    def random_particle(self):
        """ Returns a randomly initialized particle within the map. """
        # Add particles in the bounding box of the map
        ((lx,ux),(ly,uy))=self.occupancy_field.get_obstacle_bounding_box()
        width = ux-lx
        height = uy-ly
    ...
    def normalize_particles(self):
        ...
        # Divide by the total weight of the particles
